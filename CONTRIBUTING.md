...
Assuming you have [uv](https://docs.astral.sh/uv/) installed, this is how you set up your fork for local development:


    $ cd maap-documentation/
    $ make setup        # creates .venv with uv (needs Python >= 3.12; Node.js >= 20 on PATH, see .nvmrc)
...
To do this, typically you create your notebook of the example you would like added to the documentation and save the notebook under the `maap-documentation/docs/source/` directory. After the file has been saved, add it to the `toc` section of `docs/source/myst.yml` (the Table of Contents) under the right section. Prose pages are written in [MyST Markdown](https://mystmd.org/guide).
Run `$ make build` (or `$ make start` for a live-reloading server) from the repository root to build the HTML pages and verify the changes are correct. Notebook outputs are not generated by the build: run the notebook on the MAAP Hub and commit it with its outputs.
